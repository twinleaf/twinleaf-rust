...
mod tests {
    ...
    #[test]
    fn wait_capture_done_errors_when_capture_is_idle() {
        let rpc = MockCaptureRpc {
            reply: vec![CaptureStatus::Idle.as_raw()],
        };

        let err = wait_capture_done(&rpc, "test.capture", Duration::from_millis(1)).unwrap_err();

        assert!(matches!(err, CaptureError::CaptureNotStarted));
    }
    ...
    fn text_parse_metadata() {
        ...
        let meta = parse_capture_metadata(&raw).unwrap();

        assert_eq!(meta.size, 8);
        assert_eq!(meta.blocksize, 4);
        assert_eq!(meta.data_type, DataType::Float32);
        assert_eq!(meta.length, 2);
        assert_eq!(meta.y_calibration, 2.0);
        assert_eq!(meta.x_offset, 1.0);
        assert_eq!(meta.x_stride, 0.5);
        assert_eq!(meta.name, "y");
        assert_eq!(meta.units, "V");
        assert_eq!(meta.x_name, "s");
        assert_eq!(meta.x_units, "x");
    }

    #[test]
    fn decode_capture_values_applies_y_calibration() {
        ...
        raw.extend((-2.0f32).to_le_bytes());

        assert_eq!(decode_capture_values(&raw, &meta).unwrap(), vec![3.0, -4.0]);
    }

    #[test]
    fn decode_capture_values_supports_24_bit_integers() {
        ...
        assert_eq!(
            decode_capture_values(&[1, 2, 3, 0xff, 0xff, 0xff], &meta).unwrap(),
            vec![0x03_02_01 as f64, 0xff_ff_ff as f64]
        );

        meta.data_type = DataType::Int24;
        assert_eq!(
            decode_capture_values(&[0xff, 0xff, 0xff, 0, 0, 0x80], &meta).unwrap(),
            vec![-1.0, -8_388_608.0]
        );
    }

    #[test]
    fn x_values_are_derived_from_metadata_axis() {
        let meta = CaptureMetadata {
            size: 0,
            blocksize: 4,
            data_type: DataType::Float32,
        ...
        assert_eq!(meta.x_values_f64(), vec![0.25, 0.75, 1.25]);
    }
}
