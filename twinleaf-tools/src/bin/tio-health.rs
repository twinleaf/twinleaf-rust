...
fn parse_cli() -> (String, tio::proto::DeviceRoute, Cli) {
    ...
    opts.optopt(
        "",
        "mode",
        "Diagnostic mode: all|pkt|time (default all)",
        "m",
    );
    opts.optopt(
        "",
        "rate-window",
        "Seconds for rate windows (default 5)",
        "sec",
    );
    opts.optopt(
        "",
    ...
    opts.optopt(
        "",
        "rate-warn-pct",
        "Warn if smps/s dips by this % from baseline (default 0.5)",
        "%",
    );
    opts.optopt(
        "",
    ...
    opts.optopt(
        "",
        "stale-ms",
        "Grey rows after this many ms without packets (default 2000)",
        "ms",
    );
...
impl SampleStats {
    fn on_sample(&mut self, now: Instant, window: Duration) {
        ...
        if dt >= window {
            ...
            if self.rate_smps > self.baseline_smps {
                self.baseline_smps = self.rate_smps;
            }
            self.smps_in_win = 0;
            self.win_start = now;
        }
...
impl PktStats {
    ...
    fn push_diff(&mut self, diff: u32) {
        ...
        if self.diffs_len < self.diffs.len() {
            self.diffs_len += 1;
        }
        ...
        if let (Some(prev_first), Some(prev_seg)) = (self.last_first, self.last_segment) {
            if segment_id == prev_seg {
                ...
                match self.baseline_nsamp() {
                    ...
                    Some(base) => {
                        if diff > 0 && diff <= base.saturating_mul(2) {
                            self.push_diff(diff);
                        }
                    }
                }
                ...
                if diff > expected {
                    ...
                    self.skipped_packets =
                        self.skipped_packets.saturating_add(skipped_pkts_est as u64);
...
fn push_err(
    ...
    if err_log.len() >= ERR_CAP {
        err_log.pop_front();
    }
    err_log.push_back(ErrEntry {
...
fn fmt_local_pretty(t: std::time::SystemTime) -> String {
    ...
    odt_local
        .format(&fmt)
        .unwrap_or_else(|_| "<bad time>".to_string())
}
    ...
    fn draw(
        ...
        if err_recent.is_empty() && err_top.is_empty() {
            ...
            for (count, kind) in err_top.iter().take(5) {
                self.stdout
                    .queue(style::Print(format!("{:>6}  {}", count, kind)))?;
                self.stdout.queue(cursor::MoveToNextLine(1))?;
            }
            if !err_recent.is_empty() {
                ...
                for (ts, msg) in err_recent.iter().take(10) {
                    self.stdout
                        .queue(style::Print(format!("[{}]  {}", ts, msg)))?;
                    self.stdout.queue(cursor::MoveToNextLine(1))?;
                }
            }
        }
...
fn handle_packet(
    ...
    if let tio::proto::Payload::StreamData(sd) = &pkt.payload {
        ...
        ps.on_streamdata(
            sd.first_sample_n as u32,
            sd.segment_id as u32,
            now,
            rate_window,
        );
    }
...
fn main() {
    ...
    let mut port = match proxy.subtree_full(route.clone()) {
        ...
        Err(_) => {
            tui.teardown();
            eprintln!("Failed to open subtree port. Check connection / route.");
            std::process::exit(1);
        }
    };
